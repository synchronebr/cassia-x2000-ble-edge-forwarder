...
class Stats:
    def __init__(self):
        ...
        # scan de advertising
        self.scan_events_received = 0
        self.scan_raw_events = 0
        self.scan_no_mac_drops = 0
        self.scan_queue_full_drops = 0
    ...
    def snapshot(self):
        with self._lock:
            return {
                "scan_events_received": self.scan_events_received,
                "scan_raw_events": self.scan_raw_events,
                "scan_no_mac_drops": self.scan_no_mac_drops,
                "scan_queue_full_drops": self.scan_queue_full_drops,
...
def metrics_loop(stats, packet_queue, outbound_queue, scan_queue, interval_seconds):
    while not STOP:
        ...
        jlog(
            ...
            "metrics",
            "Métricas periódicas",
            scan_events_received=snap["scan_events_received"],
            scan_raw_events=snap["scan_raw_events"],
            scan_no_mac_drops=snap["scan_no_mac_drops"],
            scan_queue_depth=scan_queue.qsize(),
            scan_queue_full_drops=snap["scan_queue_full_drops"],
            connections_active=snap["connections_active"],
            connections_initiated=snap["connections_initiated"],
...
def main():
    ...
    scan_path = get_str(
        cfg,
        "scan_path",
        '/gap/nodes?event=1&filter_name=Sync&filter_duplicates=0&filter_rssi=-90&filter_value={"offset":"22","data":"04164151"}',
    )
    scan_url = gateway_api + scan_path
    ...
    tx_done_queue = Queue(maxsize=scan_queue_max)
    ...
    scan_reader = threading.Thread(
        target=scan_reader_loop,
        args=(scan_url, scan_queue, stats, lambda: STOP),
        daemon=True,
    )
    scan_reader.start()

    conn_manager = threading.Thread(
        target=connection_manager_loop,
        kwargs=dict(
            gateway_api=gateway_api,
            ...
            assembly_timeout=assembly_timeout_seconds,
            retry_margin=connection_retry_margin_seconds,
    ...
    metrics = threading.Thread(
        target=metrics_loop,
        args=(stats, packet_queue, outbound_queue, scan_queue, metrics_interval_seconds),
        daemon=True,
    )
    metrics.start()
