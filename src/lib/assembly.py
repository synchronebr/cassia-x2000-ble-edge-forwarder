...
class ReadingAssembly:
    ...
    def add_part(
        ...
        elif packet_id == BLE_TX_CRC32:
            if self.crc32_packet_no is not None:
                return False, "duplicate_crc32_packet"
            self.crc32_packet_no = packet_no
            self.crc32_payload = payload
    ...
    def missing_packets(self):
        ...
        if self.crc32_payload is None:
            missing.append({"packetType": "crc32", "packetId": BLE_TX_CRC32})
    ...
    def total_packets_expected_for_frame(self):
        ...
        if self.timestamp_payload is not None:
            total += 1
        if self.crc32_payload is not None:
            total += 1
...
def build_success_event(assembly, accel_sampling_time_ms=200, rssi=None):
    ts = try_parse_timestamp_payload(assembly.timestamp_payload)

    # Prioridade: epoch do firmware (timestamp BLE) → tempo de recepção do 1º pacote
    firmware_epoch = epoch_from_timestamp_payload(ts)
    reading_epoch  = firmware_epoch or assembly.first_packet_at or assembly.started_at
    ...
    event = {
        ...
        "readingAt": iso_utc_from_epoch(reading_epoch),
        "timestamp": ts,
        "crc32": assembly.crc32_payload.hex() if assembly.crc32_payload else None,
        "rssi": rssi,
    }
    ...
    if BLE_TX_SENSOR_IIS3DWB in assembly.sensor_parts:
        ...
        event["accel"]["samplingTimeMs"] = accel_sampling_time_ms
    ...
    if BLE_TX_SENSOR_STTS22H in assembly.sensor_parts:
        ...
        event["temp"] = decode_sensor_payload(BLE_TX_SENSOR_STTS22H, payload)["celsius"]
        ...
        if item is not None:
            ...
            if not ok:
                ...
                try:
                    ...
                    rssi = rssi_cache.get(device) if rssi_cache else None
                    evt = build_success_event(asm, accel_sampling_time_ms=accel_sampling_time_ms, rssi=rssi)
                    ...
                    if tx_done_queue is not None:
                        try:
                            tx_done_queue.put_nowait({"device": device})
                        except Full:
                            pass
